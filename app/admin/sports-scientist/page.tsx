...
export default function SportsScientistPage() {
  const [search, setSearch] = useState('')
  const [segment, setSegment] = useState<Segment>('pending')
  const { data: users = [], isLoading, isError, refetch } = useUsers()

  const counts = useMemo(() => {
    const booked = users.filter((u) => !!u.onboardingStatus?.sportsScientistBooked).length
  ...
  const segmented = useMemo(() => {
    ...
    if (segment === 'booked') {
      return users.filter((u) => !!u.onboardingStatus?.sportsScientistBooked)
    }
    return users.filter((u) => !u.onboardingStatus?.sportsScientistBooked)
  }, [users, segment])
  ...
  const emptyMessage =
    segment === 'booked'
      ? 'No members have booked the sports scientist yet.'
      : segment === 'pending'
      ? 'No pending members for the sports scientist step.'
      : 'No members found.'
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Sports Scientist</h2>
          <p className="text-muted-foreground">
            Members segmented by sports scientist onboarding state
          </p>
        </div>
      </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filtered.length === 0 ? (
                        ...
                        filtered.map((user) => {
                          const booked = !!user.onboardingStatus?.sportsScientistBooked

...
function NutritionWorkspace({
  ...
  const profile: NutritionProfile = useMemo(() => {
    ...
    const rawWeight = selectedUser.healthMarkers?.weight ?? latestWeightKg
    const rawHeight = selectedUser.healthMarkers?.height
    return {
      weight:   rawWeight != null && !Number.isNaN(Number(rawWeight)) ? Number(rawWeight) : null,
      height:   rawHeight != null && !Number.isNaN(Number(rawHeight)) ? Number(rawHeight) : null,

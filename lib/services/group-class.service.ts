...
function normalizeGroupClass(raw: any): GroupClass {
  ...
  return {
    ...
    videoRoomId: raw?.videoRoomId || raw?.zegoRoomId || raw?.videoConferenceId || raw?._id || raw?.id || '',
    zegoRoomId: raw?.videoRoomId || raw?.zegoRoomId || raw?.videoConferenceId || raw?._id || raw?.id || '',
